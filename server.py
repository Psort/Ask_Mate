...
def users_list():
    ...
    headers = ['id', 'User name', 'Registration date', 'Asked questions', 'Answers',
               'Comments', 'Reputation']
    return render_template('user_list.html', user_list=users_container, headers=headers)

@app.route('/users/<int:user_id>')
def user_info(user_id):
    user_inf = data_manager.get_single_user_info(user_id)
    user_quest = data_manager.get_user_questions(user_id)
    user_ans = data_manager.get_user_answers(user_id)
    user_comments = data_manager.get_user_comments(user_id)

    com_head = ['Comment', 'Id']
    answer_headers = ['Answer', 'Vote number', 'Id']
    question_headers = ['Title', 'Question', 'View number', 'Vote number', 'Id']
    return render_template('user_list.html', user_info=user_inf, question_headers=question_headers,
                           user_quest=user_quest, user_ans=user_ans, ans_head=answer_headers,
                           com_head=com_head, user_comments= user_comments, switch= 1)
